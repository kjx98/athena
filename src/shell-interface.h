...
struct ShellExternalInterface : ModuleInstance::ExternalInterface {
  ...
  public:
    Memory() {}
    // Gives no guarantee about the length of the memory. Caller needs to ensure
    // that.
    char *rawpointer(size_t offset) { return &memory[offset]; }
    ...
    for (auto &segment : wasm.memory.segments) {
      Address offset = static_cast<uint32_t>(
          ConstantExpressionRunner<TrivialGlobalManager>(instance.globals)
              ...
              .value.geti32());
      assert(offset + segment.data.size() <=
  ...
  Literal callTable(Index index, LiteralList &arguments, Type result,
