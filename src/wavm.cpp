...
namespace wavm_host_module {
  ...
  struct AthenaWavmResolver : Runtime::Resolver {
    ...
    ) override
    {
      ...
      if (outObject && !isA(outObject, type))
        outObject = nullptr;
      H_DEBUG << "Resolving " << moduleName << " : " << exportName << " -> " << (outObject != nullptr) << "\n";
...
ExecutionResult WavmEngine::internalExecute(
  ...
  athenaAssert(ethereumHostModule, "Failed to create host module.");

  // prepare contract module to resolve links against host module
  wavm_host_module::AthenaWavmResolver resolver;
  ...
  athenaAssert(module, "Couldn't compile IR to bitcode.");
  ...
  athenaAssert(moduleInstance, "Couldn't instantiate contact module.");
