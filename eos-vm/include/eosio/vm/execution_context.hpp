...
template <typename Derived, typename Host> class execution_context_base {
public:
  ...
  inline int32_t grow_linear_memory(int32_t pages) {
    ...
    if (pages < 0) {
      ...
      if (!_mod.memories.size() || max_pages - sz < pages ||
          (_mod.memories[0].limits.flags &&
           (static_cast<int32_t>(_mod.memories[0].limits.maximum) - sz <
            pages)))
        return -1;
  ...
  inline void reset() {
    ...
    for (uint32_t i = 0; i < _mod.data.size(); i++) {
      ...
      auto available_memory =
          _mod.memories[0].limits.initial * static_cast<uint64_t>(page_size);
      auto required_memory =
          static_cast<uint64_t>(offset) + data_seg.data.size();
      EOS_VM_ASSERT(required_memory <= available_memory, wasm_memory_exception,
                    "data out of range");
  ...
  inline std::optional<operand_stack_elem>
  execute(Host *host, Visitor &&visitor, const std::string_view func,
          Args... args) {
    uint32_t func_index = _mod.get_exported_function(func);
    return derived().execute(host, std::forward<Visitor>(visitor), func_index,
...
class jit_execution_context
    : public execution_context_base<jit_execution_context<Host>, Host> {
...
public:
  ...
  inline native_value call_host_function(native_value *stack, uint32_t index) {
    ...
    for (uint32_t i = 0; i < ft.param_types.size(); ++i) {
      ...
      case i32:
        _os.push(i32_const_t{stack[num_params - i - 1].i32});
        break;
      case i64:
        _os.push(i64_const_t{stack[num_params - i - 1].i64});
        break;
      case f32:
        _os.push(f32_const_t{stack[num_params - i - 1].f32});
        break;
      case f64:
        _os.push(f64_const_t{stack[num_params - i - 1].f64});
        break;
      default:
    ...
    auto set_result = [&result](auto val) {
  ...
  inline std::optional<operand_stack_elem>
  execute(Host *host, jit_visitor, uint32_t func_index, Args... args) {
    auto saved_host = _host;
    auto saved_os_size = _os.size();
    auto g = scope_guard([&]() {
      _host = saved_host;
      _os.eat(saved_os_size);
    });
    ...
    try {
      if (func_index < _mod.get_imported_functions_size()) {
        ...
        std::size_t maximum_stack_usage =
            (_mod.maximum_stack + 2 /*frame ptr + return ptr*/) *
                (constants::max_call_depth + 1) +
        ...
        auto fn = reinterpret_cast<native_value (*)(void *, void *)>(
            _mod.code[func_index - _mod.get_imported_functions_size()]
                .jit_code_offset +
            _mod.allocator._code_base);

        vm::invoke_with_signal_handler(
            [&]() {
              result = execute<sizeof...(Args)>(args_raw, fn, this,
                                                _linear_memory, stack);
            },
            &handle_signal);
      }
...
protected:
  template <typename T> native_value transform_arg(T &&value) {
    ...
    auto transformed_value =
        detail::resolve_result(static_cast<T &&>(value), this->_wasm_alloc)
            .data;
  ...
  template <int Count>
  static native_value
  execute(native_value *data, native_value (*fun)(void *, void *),
          jit_execution_context *context, void *linear_memory, void *stack) {
    static_assert(sizeof(native_value) == 8,
    ...
    asm volatile(
        ...
        "ldmxcsr 16(%%rsp); "
        "mov (%%rsp), %%rsp; "
        // Force explicit register allocation, because otherwise it's too hard
        // to get the clobbers right.
        : [ result ] "=&a"(result), // output, reused as a scratch register
          [ data ] "+d"(data), [ fun ] "+c"(fun),
          [ stack_top ] "+r"(stack_top) // input only, but may be clobbered
        : [ context ] "D"(context), [ linear_memory ] "S"(linear_memory),
          [ StackOffset ] "n"(Count * 8), [ Count ] "n"(Count),
          "b"(stack_check) // input
        : "memory",
          "cc", // clobber
          ...
          "mm0", "mm1", "mm2", "mm3", "mm4", "mm5", "mm6", "mm6", "st", "st(1)",
  ...
  bounded_allocator _base_allocator = {constants::max_stack_size *
                                       sizeof(operand_stack_elem)};
  operand_stack _os;
};

template <typename Host>
class execution_context
    : public execution_context_base<execution_context<Host>, Host> {
...
public:
  ...
  inline void call(uint32_t index) {
    ...
    if (index < _mod.get_imported_functions_size()) {
      ...
      // const auto& ft = _mod.types[_mod.functions[index -
      // _mod.get_imported_functions_size()]]; type_check(ft);
      push_call(index);
      setup_locals(index);
  ...
  void print_stack() {
    ...
    for (int i = 0; i < _os.size(); i++) {
      ...
      visit(
          overloaded{
              [&](i32_const_t el) {
                std::cout << "i32:" << el.data.ui << ", ";
              },
              [&](i64_const_t el) {
                std::cout << "i64:" << el.data.ui << ", ";
              },
  ...
  inline operand_stack_elem get_operand(uint16_t index) const {
    return _os.get(_last_op_index + index);
  }
  inline void eat_operands(uint16_t index) { _os.eat(index); }
  inline void compact_operand(uint16_t index) { _os.compact(index); }
  inline void set_operand(uint16_t index, const operand_stack_elem &el) {
    _os.set(_last_op_index + index, el);
  }
  ...
  template <bool Should_Exit = false> inline void push_call(uint32_t index) {
    ...
    _last_op_index =
        _os.size() - _mod.get_function_type(index).param_types.size();
  }
  ...
  inline operand_stack_elem get_global(uint32_t index) {
    EOS_VM_ASSERT(index < _mod.globals.size(), wasm_interpreter_exception,
                  "global index out of range");
    const auto &gl = _mod.globals[index];
    switch (gl.type.content_type) {
    case types::i32:
      return i32_const_t{*(uint32_t *)&gl.current.value.i32};
    case types::i64:
      return i64_const_t{*(uint64_t *)&gl.current.value.i64};
    case types::f32:
  ...
  inline void type_check(const func_type &ft) {
    for (uint32_t i = 0; i < ft.param_types.size(); i++) {
      ...
      visit(overloaded{[&](const i32_const_t &) {
                         EOS_VM_ASSERT(ft.param_types[i] == types::i32,
                                       wasm_interpreter_exception,
                                       "function param type mismatch");
                       },
                       [&](const f32_const_t &) {
                         EOS_VM_ASSERT(ft.param_types[i] == types::f32,
                                       wasm_interpreter_exception,
                                       "function param type mismatch");
                       },
                       [&](const i64_const_t &) {
                         EOS_VM_ASSERT(ft.param_types[i] == types::i64,
                                       wasm_interpreter_exception,
                                       "function param type mismatch");
                       },
                       [&](const f64_const_t &) {
                         EOS_VM_ASSERT(ft.param_types[i] == types::f64,
                                       wasm_interpreter_exception,
                                       "function param type mismatch");
                       },
                       [&](auto) {
  ...
  template <typename Visitor, typename... Args>
  inline std::optional<operand_stack_elem>
  execute_func_table(Host *host, Visitor &&visitor, uint32_t table_index,
                     Args... args) {
    return execute(host, std::forward<Visitor>(visitor),
                   table_elem(table_index), std::forward<Args>(args)...);
  }
  ...
  execute(Host *host, Visitor &&visitor, const std::string_view func,
          Args... args) {
    uint32_t func_index = _mod.get_exported_function(func);
    return execute(host, std::forward<Visitor>(visitor), func_index,
                   std::forward<Args>(args)...);
  }
  ...
  execute(Host *host, Visitor &&visitor, uint32_t func_index, Args... args) {
    EOS_VM_ASSERT(func_index < std::numeric_limits<uint32_t>::max(),
                  wasm_interpreter_exception,
    ...
    if (func_index < _mod.get_imported_functions_size()) {
      ...
      vm::invoke_with_signal_handler([&]() { execute(visitor); },
                                     &handle_signal);
    }
...
#define CREATE_LABEL(NAME, CODE)                                               \
  ev_label_##NAME                                                              \
      : visitor(ev_variant->template get<eosio::vm::EOS_VM_OPCODE_T(NAME)>()); \
...
#define CREATE_EMPTY_LABEL(NAME, CODE)                                         \
  ev_label_##NAME : throw wasm_interpreter_exception{"empty operand"};

  template <typename Visitor> void execute(Visitor &&visitor) {
    static void *dispatch_table[] = {
        EOS_VM_CONTROL_FLOW_OPS(CREATE_TABLE_ENTRY) EOS_VM_BR_TABLE_OP(
            CREATE_TABLE_ENTRY) EOS_VM_RETURN_OP(CREATE_TABLE_ENTRY)
            EOS_VM_CALL_OPS(CREATE_TABLE_ENTRY) EOS_VM_CALL_IMM_OPS(
                CREATE_TABLE_ENTRY) EOS_VM_PARAMETRIC_OPS(CREATE_TABLE_ENTRY)
                EOS_VM_VARIABLE_ACCESS_OPS(
                    CREATE_TABLE_ENTRY) EOS_VM_MEMORY_OPS(CREATE_TABLE_ENTRY)
...
#undef CREATE_TABLE_ENTRY
  ...
  bounded_allocator _base_allocator = {
      (constants::max_stack_size + constants::max_call_depth + 1) *
      (std::max(sizeof(operand_stack_elem), sizeof(activation_frame)))};
