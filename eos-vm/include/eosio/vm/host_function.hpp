...
namespace eosio {
namespace chain {
template <typename T> struct array_ptr;
}
} // namespace eosio
...
template <typename Derived, typename Base> struct construct_derived {
  static auto value(Base &base) { return Derived(base); }
  typedef Base type;
};

template <typename Derived> struct construct_derived<Derived, nullptr_t> {
...
template <class T, class U> inline constexpr U &&make_dependent(U &&u) {
...
template <typename R, typename... Args> auto get_return_t(R(Args...)) -> R;
...
template <typename T> void validate_ptr(const void *ptr, uint32_t len) {
  EOS_VM_ASSERT(len <= std::numeric_limits<std::uint32_t>::max() /
                           (uint32_t)sizeof(T),
...
template <typename T, std::size_t Align> struct aligned_ptr_wrapper {
  static_assert(Align % alignof(T) == 0,
                "Must align to at least the alignment of T");
...
template <typename T, std::size_t Align> struct aligned_array_wrapper {
  static_assert(Align % alignof(T) == 0,
                "Must align to at least the alignment of T");
...
template <typename T, std::size_t Align> struct aligned_ref_wrapper {
  constexpr aligned_ref_wrapper(void *ptr) : _impl(ptr) {}
  constexpr operator T &() const { return *static_cast<T *>(_impl); }

  aligned_ptr_wrapper<T, Align> _impl;
};

template <typename T, std::size_t Align = alignof(T)> struct aligned_ptr {
  aligned_ptr(const aligned_ptr_wrapper<T, Align> &wrapper) : _ptr(wrapper) {}
  operator T *() const { return _ptr; }
  T *_ptr;
};

template <typename T, std::size_t Align = alignof(T)> struct aligned_ref {
...
template <typename T> struct wasm_type_converter {};
...
template <typename T>
using from_wasm_type_impl_t =
    decltype(detail::from_wasm_type_impl(&wasm_type_converter<T>::from_wasm));

template <typename T, typename U, typename... Args>
auto to_wasm_type_impl(T (*)(U, Args...)) -> T;
template <typename T>
using to_wasm_type_impl_t =
    decltype(detail::to_wasm_type_impl(&wasm_type_converter<T>::to_wasm));
...
template <typename T>
struct get_wasm_type<T, std::void_t<from_wasm_type_impl_t<T>>,
                     std::void_t<to_wasm_type_impl_t<T>>> {
  static_assert(
      std::is_same_v<from_wasm_type_impl_t<T>, to_wasm_type_impl_t<T>>,
      "wasm_type_converter must use the same type for both from_wasm and "
      "to_wasm.");
  using type = from_wasm_type_impl_t<T>;
};
...
template <typename T, typename StorageType> struct cons_item {
  template <typename F> explicit cons_item(F &&f) : _value(f()) {}
  StorageType _value;
...
template <> struct cons<void, void> { static constexpr std::size_t size = 0; };
...
template <typename A, typename SourceType, typename WAlloc, typename Tail,
          typename T, std::size_t... Is>
decltype(auto) get_value_impl(std::index_sequence<Is...>, WAlloc *alloc,
                              T &&val, const Tail &tail) {
  return detail::init_wasm_type_converter(wasm_type_converter<A>{}, alloc)
      .from_wasm(get_value<SourceType>(alloc, static_cast<T &&>(val), tail),
                 cons_get<Is>(tail)...);
}

// Matches a specific overload of a function and deduces the first argument
template <typename... Rest> struct match_from_wasm {
  template <typename R, typename U> static U apply(R(U, Rest...));
...
// Encodes how to convert a value from wasm to native.  LookaheadCount is the
// number of extra arguments to pass to from_wasm.  SourceType is type to
// convert from.
template <std::size_t LookaheadCount, typename SourceType> struct value_getter {
  template <typename A, typename WAlloc, typename T, typename Tail>
  decltype(auto) apply(WAlloc *alloc, T &&val, const Tail &tail) {
    return get_value_impl<A, SourceType>(
        std::make_index_sequence<LookaheadCount>{}, alloc,
        static_cast<T &&>(val), tail);
  }
};
...
auto try_value_getter(std::index_sequence<Is...>)
    -> value_getter<sizeof...(Is),
                    decltype(match_from_wasm<cons_item_t<Is, Cons>...>::apply(
                        &wasm_type_converter<T>::from_wasm))>;
// Fallback
template <typename T, typename Cons> auto try_value_getter(...) -> void;

template <std::size_t N, typename T, typename Cons>
using try_value_getter_t =
    decltype(try_value_getter<T, Cons>(std::make_index_sequence<N>{}));
...
template <typename T, typename Tail> constexpr auto make_value_getter() {
...
template <typename Args> struct pack_args;

template <typename T, typename F> auto make_cons_item(F &&f) {
  return [=](auto &arg) {
    return cons_item<T, decltype(f(arg))>{
        [&]() -> decltype(auto) { return f(arg); }};
  };
}

template <typename T0, typename... T> struct pack_args<std::tuple<T0, T...>> {
  ...
  static auto apply(WAlloc *alloc, Os &os) {
    ...
    auto item_maker = make_cons_item<T0>([&](auto &tail) -> decltype(auto) {
      return get_value<T0>(alloc, os.get_back(sizeof...(T)), tail);
    });
    using result_type =
        cons<decltype(item_maker(std::declval<next_result &>())), next_result>;
    return result_type(item_maker, [&]() { return next::apply(alloc, os); });
  }
};

template <> struct pack_args<std::tuple<>> {
  template <typename WAlloc, typename Os>
  static auto apply(WAlloc *alloc, Os &os) {
    return nil_t{};
  }
...
template <typename T> std::decay_t<T> as_value(T &&arg) {
...
constexpr decltype(auto) get_value(WAlloc *alloc, T &&val, Cons &tail) {
  if constexpr (std::is_integral_v<S> && sizeof(S) == 4)
    ...
    return reinterpret_cast<S>(alloc->template get_base_ptr<char>() +
                               val.template get<i32_const_t>().data.ui);
  else {
    return detail::make_value_getter<S, Cons>().template apply<S>(
        alloc, static_cast<T &&>(val), tail);
  }
}
...
constexpr auto resolve_result(T &&res, WAlloc *alloc) {
  if constexpr (std::is_integral_v<T> && sizeof(T) == 4)
    ...
    return i32_const_t{static_cast<uint32_t>(
        reinterpret_cast<uintptr_t>(res) -
        reinterpret_cast<uintptr_t>(alloc->template get_base_ptr<char>()))};
  else
    return detail::resolve_result(
        detail::init_wasm_type_converter(wasm_type_converter<T>{}, alloc)
            .to_wasm(static_cast<T &&>(res)),
        alloc);
}
} // namespace detail

template <> struct wasm_type_converter<bool> {
...
template <typename T> inline constexpr auto to_wasm_type() -> uint8_t {
...
template <uint8_t Type> struct _to_wasm_t;

template <> struct _to_wasm_t<types::i32> { typedef i32_const_t type; };

template <> struct _to_wasm_t<types::i64> { typedef i64_const_t type; };

template <> struct _to_wasm_t<types::f32> { typedef f32_const_t type; };

template <> struct _to_wasm_t<types::f64> { typedef f64_const_t type; };
...
decltype(auto) invoke_with_host(Host *host, T &&... args) {
  if constexpr (std::is_same_v<Derived, nullptr_t>)
    ...
    return std::invoke(F, construct_derived<Derived, Host>::value(*host),
                       static_cast<T &&>(args)...);
}

template <auto F, typename Derived, typename Host, typename Cons,
          std::size_t... Is>
decltype(auto) invoke_with_cons(Host *host, Cons &&args,
...
template <typename Walloc, typename Cls, typename Cls2, auto F, typename R,
...
auto create_function(std::index_sequence<Is...>) {
  return std::function<void(Cls *, Walloc *, operand_stack &)>{
      [](Cls *self, Walloc *walloc, operand_stack &os) {
        maybe_push_result((invoke_with_cons<F, Cls2>(
                               self, detail::pack_args<Args>::apply(walloc, os),
                               std::index_sequence<Is...>{}),
                           maybe_void),
                          os, walloc, sizeof...(Is));
      }};
...
template <typename T> constexpr auto to_wasm_type_v = to_wasm_type<T>();
...
template <typename Cls> struct registered_host_functions {
  template <typename WAlloc> struct mappings {
  ...
  template <typename WAlloc> static mappings<WAlloc> &get_mappings() {
  ...
  static void add(const std::string &mod, const std::string &name) {
    using deduced_full_ts =
        decltype(get_args_full(AUTO_PARAM_WORKAROUND(Func)));
    using res_t = decltype(get_return_t(AUTO_PARAM_WORKAROUND(Func)));
    static constexpr auto is =
        std::make_index_sequence<std::tuple_size_v<deduced_full_ts>>();
    auto &current_mappings = get_mappings<WAlloc>();
    current_mappings.named_mapping[{mod, name}] =
        current_mappings.current_index++;
    current_mappings.functions.push_back(
        create_function<WAlloc, Cls, Cls2, Func, res_t, deduced_full_ts>(is));
  }

  template <typename Module> static void resolve(Module &mod) {
    auto &imports = mod.import_functions;
    auto &current_mappings = get_mappings<wasm_allocator>();
    for (int i = 0; i < mod.imports.size(); i++) {
      std::string mod_name =
          std::string((char *)mod.imports[i].module_str.raw(),
                      mod.imports[i].module_str.size());
      std::string fn_name = std::string((char *)mod.imports[i].field_str.raw(),
                                        mod.imports[i].field_str.size());
      EOS_VM_ASSERT(current_mappings.named_mapping.count({mod_name, fn_name}),
                    wasm_link_exception, "no mapping for imported function");
...
template <typename Cls, typename Cls2, auto F> struct registered_function {
  registered_function(std::string mod, std::string name) {
    registered_host_functions<Cls>::template add<Cls2, F, wasm_allocator>(mod,
                                                                          name);
  }
};
