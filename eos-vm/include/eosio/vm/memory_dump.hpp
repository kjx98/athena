...
template <typename Stream> struct memory_dump_visitor {
  ...
  template <typename T> inline void operator()(T) {
    stream << "invalid opcode\n";
  }
  Stream &stream;
  size_t &index;
};

template <typename Opcode> class memory_dump {
public:
