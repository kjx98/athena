...
struct jit {
  ...
  using parser =
      binary_parser<machine_code_writer<jit_execution_context<Host>>>;
  static constexpr bool is_jit = true;
};

struct interpreter {
  template <typename Host> using context = execution_context<Host>;
  template <typename Host> using parser = binary_parser<bitcode_writer>;
  static constexpr bool is_jit = false;
};

template <typename Host, typename Impl = interpreter> class backend {
public:
  using host_t = Host;

  template <typename HostFunctions = nullptr_t>
  backend(wasm_code &code, HostFunctions = nullptr)
      : _ctx(typename Impl::template parser<Host>{_mod.allocator}.parse_module(
            code, _mod)) {
  ...
  template <typename HostFunctions = nullptr_t>
  backend(wasm_code_ptr &ptr, size_t sz, HostFunctions = nullptr)
      : _ctx(typename Impl::template parser<Host>{_mod.allocator}.parse_module2(
            ptr, sz, _mod)) {
    if constexpr (!std::is_same_v<HostFunctions, nullptr_t>)
      HostFunctions::resolve(_mod);
    _mod.finalize();
  ...
  inline bool operator()(Host *host, const std::string_view &mod,
                         const std::string_view &func, Args... args) {
  ...
  inline bool call_indirect(Host *host, uint32_t func_index, Args... args) {
    try {
      if constexpr (eos_vm_debug) {
        //_ctx.execute_func_table(host, debug_visitor(_ctx), func_index,
        // args...);
        _ctx.execute_func_table(host, interpret_visitor(_ctx), func_index,
                                args...);
      } else {
        _ctx.execute_func_table(host, interpret_visitor(_ctx), func_index,
                                args...);
  ...
  template <typename... Args>
  inline bool call(Host *host, const std::string_view &mod,
                   const std::string_view &func, Args... args) {
    if constexpr (eos_vm_debug) {
  ...
  template <typename... Args>
  inline auto call_with_return(Host *host, const std::string_view &mod,
                               const std::string_view &func, Args... args) {
    try {
      if constexpr (eos_vm_debug) {
    ...
    timed_run(static_cast<Watchdog &&>(wd), [&]() {
      for (int i = 0; i < _mod.exports.size(); i++) {
        if (_mod.exports[i].kind == external_kind::Function) {
          std::string s{(const char *)_mod.exports[i].field_str.raw(),
                        _mod.exports[i].field_str.size()};
          if constexpr (eos_vm_debug) {
