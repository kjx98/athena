...
public:
  ...
  /// be called.
  template <typename F>[[nodiscard]] guard scoped_run(F &&callback) {
...
private:
  ...
  public:
    ...
    guard(const TimeUnits &duration, F &&callback)
        : _callback(static_cast<F &&>(callback)), _run_state(running),
          _duration(duration), _start(std::chrono::steady_clock::now()) {
  ...
  private:
    ...
    void runner() {
      ...
      _cond.wait_until(_lock, _start + _duration,
...
class null_watchdog {
public:
  template <typename F> null_watchdog scoped_run(F &&) { return *this; }
