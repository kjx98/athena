...
public:
  ...
  template <typename T> T *alloc(size_t size = 1) {
    EOS_VM_ASSERT((sizeof(T) * size) + index <= mem_size, wasm_bad_alloc,
                  "wasm bounded_allocator failed to allocate native");
...
public:
  ...
  contiguous_allocator(size_t size) {
    ...
    EOS_VM_ASSERT(_base != MAP_FAILED, wasm_bad_alloc,
                  "contiguous_allocator mmap failed.");
  }
  ...
  template <typename T> T *alloc(size_t size = 0) {
    ...
    if (aligned > _size) {
      ...
      char *new_base = (char *)mmap(NULL, new_size, PROT_READ | PROT_WRITE,
                                    MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
...
public:
  ...
  void free(void *ptr) noexcept {
    ...
    transfer_node(allocated_blocks_by_size, free_blocks_by_size,
...
private:
  struct segment {
    segment(void *bas, std::size_t siz) : base(bas), size(siz) {}
  ...
  struct by_size {
    ...
    bool operator()(const block &lhs, const block &rhs) const {
      return lhs.first < rhs.first ||
             (lhs.first == rhs.first &&
              std::less<void *>{}(lhs.second, rhs.second));
    }
  ...
  template <typename C>
  static typename C::iterator transfer_node(C &from, C &to,
                                            typename C::key_type key) noexcept {
  ...
  blocks_by_size_t::iterator allocate_segment(std::size_t min_size) {
    ...
    void *base =
        mmap(nullptr, size, PROT_EXEC, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    segment s{base, size};
    EOS_VM_ASSERT(base != MAP_FAILED, wasm_bad_alloc,
  ...
  blocks_by_size_t::iterator split_block(blocks_by_size_t::iterator pos,
                                         std::size_t size) {
    bool success = false;
    ...
    auto new2 = free_blocks_by_size
                    .insert({pos->first - size,
                             static_cast<char *>(pos->second) + size})
                    .first;
  ...
  blocks_t::iterator maybe_consolidate_blocks(blocks_t::iterator lhs,
                                              blocks_t::iterator rhs) noexcept {
...
public:
  ...
  growable_allocator(size_t size) {
    EOS_VM_ASSERT(size <= max_memory_size, wasm_bad_alloc,
                  "Too large initial memory size");
    _base = (char *)mmap(NULL, max_memory_size, PROT_NONE,
                         MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    EOS_VM_ASSERT(_base != MAP_FAILED, wasm_bad_alloc,
                  "growable_allocator mmap failed.");
    if (size != 0) {
  ...
  template <typename T> T *alloc(size_t size = 0) {
    static_assert(max_memory_size % alignof(T) == 0,
    ...
    EOS_VM_ASSERT((max_memory_size - _offset) / sizeof(T) >= size,
                  wasm_bad_alloc, "Allocated too much memory");
    size_t aligned = (sizeof(T) * size) + _offset;
    if (aligned > _size) {
      size_t chunks_to_alloc =
          align_offset<chunk_size>(aligned - _size) / chunk_size;
      mprotect((char *)_base + _size, (chunk_size * chunks_to_alloc),
  ...
  void enable_code(bool is_jit_) {
    mprotect(_code_base, _code_size,
             is_jit_ ? PROT_EXEC : (PROT_READ | PROT_WRITE));
  }
  // Make code pages unexecutable
  void disable_code() { mprotect(_code_base, _code_size, PROT_NONE); }

  /* different semantics than free,
   * the memory must be at the end of the most recently allocated block.
   */
  template <typename T> void reclaim(const T *ptr, size_t size = 0) {
    EOS_VM_ASSERT(_offset / sizeof(T) >= size, wasm_bad_alloc,
                  "reclaimed too much memory");
    EOS_VM_ASSERT(size == 0 || (char *)(ptr + size) == (_base + _offset),
                  wasm_bad_alloc, "reclaiming memory must be strictly LIFO");
    if (size != 0)
      _offset = ((char *)ptr - _base);
  }

  /*
   * Finalize the memory by unmapping any excess pages, this means that the
   * allocator will no longer grow
   */
  void finalize() {
    if (_capacity != _offset) {
      std::size_t final_size = align_to_page(_offset);
      EOS_VM_ASSERT(munmap(_base + final_size, _capacity - final_size) == 0,
                    wasm_bad_alloc, "failed to finalize growable_allocator");
...
template <typename T> class fixed_stack_allocator {
private:
  T *raw = nullptr;
  size_t max_size = 0;

public:
  template <typename U> void free() { munmap(raw, max_memory); }
  fixed_stack_allocator(size_t max_siz) : max_size(max_siz) {
    raw = (T *)mmap(NULL, max_memory, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS,
                    -1, 0);
    EOS_VM_ASSERT(raw != MAP_FAILED, wasm_bad_alloc,
...
public:
  template <typename T> void alloc(size_t size = 1 /*in pages*/) {
    ...
    EOS_VM_ASSERT(size <= max_pages - page, wasm_bad_alloc,
                  "wasm_allocator exceeded max number of pages");
    int err = mprotect(raw + (page_size * page), (page_size * size),
                       PROT_READ | PROT_WRITE);
    EOS_VM_ASSERT(err == 0, wasm_bad_alloc, "mprotect failed");
  ...
  wasm_allocator() {
    std::size_t syspagesize = static_cast<std::size_t>(::sysconf(_SC_PAGESIZE));
    raw = (char *)mmap(NULL, max_memory + 2 * syspagesize, PROT_NONE,
                       MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
    EOS_VM_ASSERT(raw != MAP_FAILED, wasm_bad_alloc,
                  "wasm_allocator mmap failed to alloca pages");
  ...
  void reset(uint32_t new_pages) {
    ...
    if (new_pages != page && page > 0) {
      int err = mprotect(raw, page_size * page,
                         PROT_NONE); // protect the entire region of memory
      EOS_VM_ASSERT(err == 0, wasm_bad_alloc, "mprotect failed");
    }
    page = 0;
  }
  ...
  void reset() {
    if (page != -1) {
      std::size_t syspagesize =
          static_cast<std::size_t>(::sysconf(_SC_PAGESIZE));
      memset(raw, '\0', page_size * page); // zero the memory
      int err = mprotect(raw - syspagesize, page_size * page + syspagesize,
                         PROT_NONE);
      EOS_VM_ASSERT(err == 0, wasm_bad_alloc, "mprotect failed");
    }
    page = -1;
  }
  template <typename T> inline T *get_base_ptr() const {
