...
inline bool operator==(const func_type &lhs, const func_type &rhs) {
  return lhs.form == rhs.form &&
         ...
         std::equal(lhs.param_types.raw(),
                    lhs.param_types.raw() + lhs.param_types.size(),
                    rhs.param_types.raw()) &&
...
struct module {
  ...
  inline opcode *get_function_pc(uint32_t fidx) const {
    EOS_VM_ASSERT(fidx >= get_imported_functions_size(),
                  wasm_interpreter_exception,
                  "trying to get the PC of an imported function");
  ...
  inline uint32_t get_function_locals_size(uint32_t index) const {
    EOS_VM_ASSERT(index >= get_imported_functions_size(),
                  wasm_interpreter_exception,
                  "imported functions do not have locals");
  ...
  uint32_t get_exported_function(const std::string_view str) {
    ...
    for (uint32_t i = 0; i < exports.size(); i++) {
      if (exports[i].kind == external_kind::Function &&
          ...
          memcmp((const char *)str.data(),
                 (const char *)exports[i].field_str.raw(),
                 exports[i].field_str.size()) == 0) {
