...
inline thread_local std::atomic<sigjmp_buf *> signal_dest =
...
inline void signal_handler(int sig, siginfo_t *info, void *uap) {
  ...
  if (dest) {
    ...
    switch (sig) {
    case SIGSEGV:
      prev_action = &prev_signal_handler<SIGSEGV>;
      break;
    case SIGBUS:
      prev_action = &prev_signal_handler<SIGBUS>;
      break;
    case SIGFPE:
...
///
// Make this noinline to prevent possible corruption of the caller's local
// variables. It's unlikely, but I'm not sure that it can definitely be ruled
// out if both this and f are inlined and f modifies locals from the caller.
