...
template <bool Valid, typename Ret> struct dispatcher;

template <typename Ret> struct dispatcher<false, Ret> {
...
template <typename Ret> struct dispatcher<true, Ret> {
  template <std::size_t I, typename Vis, typename Var>
  static constexpr Ret _case(Vis &&vis, Var &&var) {
    return std::invoke(std::forward<Vis>(vis),
                       std::forward<Var>(var).template get<I>());
  }
  ...
  static constexpr Ret _switch(Vis &&vis, Var &&var) {
    ...
    switch (var.index()) {
    case I + 0: {
      return dispatcher<I + 0 < sz, Ret>::template _case<I + 0>(
          std::forward<Vis>(vis), std::forward<Var>(var));
    }
    case I + 1: {
      return dispatcher<I + 1 < sz, Ret>::template _case<I + 1>(
          std::forward<Vis>(vis), std::forward<Var>(var));
    }
    case I + 2: {
      return dispatcher<I + 2 < sz, Ret>::template _case<I + 2>(
          std::forward<Vis>(vis), std::forward<Var>(var));
    }
    case I + 3: {
      return dispatcher<I + 3 < sz, Ret>::template _case<I + 3>(
          std::forward<Vis>(vis), std::forward<Var>(var));
    }
    default: {
      return dispatcher<I + 4 < sz, Ret>::template _switch<I + 4>(
          std::forward<Vis>(vis), std::forward<Var>(var));
    }
    }
  }
...
constexpr decltype(auto) variant_storage_get(Storage &&val) {
  if constexpr (I == 0) {
    ...
    return detail::variant_storage_get<I - 4>(
        static_cast<Storage &&>(val)._tail);
  }
}
} // namespace detail
...
constexpr auto visit(Visitor &&vis, Variant &&var) {
  ...
  return detail::dispatcher<true, Ret>::template _switch<0>(
      std::forward<Visitor>(vis), std::forward<Variant>(var));
}

template <typename... Alternatives> class variant {
  static_assert(sizeof...(Alternatives) <=
                    std::numeric_limits<uint8_t>::max() + 1,
                "eosio::vm::variant can only accept 256 alternatives");
  static_assert((... && (std::is_trivially_copy_constructible_v<Alternatives> &&
                         std::is_trivially_move_constructible_v<Alternatives> &&
                         std::is_trivially_copy_assignable_v<Alternatives> &&
                         std::is_trivially_move_assignable_v<Alternatives> &&
                         std::is_trivially_destructible_v<Alternatives>)),
...
public:
  ...
  template <typename T,
            typename = std::enable_if_t<detail::is_valid_alternative_v<
                std::decay_t<T>, Alternatives...>>>
  constexpr variant(T &&alt)
      : _which(
            detail::get_alternatives_index_v<std::decay_t<T>, Alternatives...>),
  ...
  static inline constexpr size_t variant_size() {
    return sizeof...(Alternatives);
  }
  inline constexpr uint16_t index() const { return _which; }

#ifdef ommit
  template <size_t Index> inline constexpr auto &&get_check() {
    // TODO add outcome stuff
    return 3;
  }
#endif

  template <size_t Index> inline constexpr const auto &get() const & {
    return detail::variant_storage_get<Index>(_storage);
  }

  template <typename Alt> inline constexpr const Alt &get() const & {
    return detail::variant_storage_get<
        detail::get_alternatives_index_v<Alt, Alternatives...>>(_storage);
  }
  ...
  template <typename Alt> inline constexpr const Alt &&get() const && {
    return detail::variant_storage_get<
        detail::get_alternatives_index_v<Alt, Alternatives...>>(
        std::move(_storage));
  }

  template <size_t Index> inline constexpr auto &&get() && {
    return detail::variant_storage_get<Index>(std::move(_storage));
  }

  template <typename Alt> inline constexpr Alt &&get() && {
    return detail::variant_storage_get<
        detail::get_alternatives_index_v<Alt, Alternatives...>>(
        std::move(_storage));
  }
  ...
  template <typename Alt> inline constexpr bool is_a() const {
