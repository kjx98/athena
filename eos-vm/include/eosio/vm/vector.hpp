...
public:
  ...
  template <typename U, typename = std::enable_if_t<
                            std::is_same_v<T, std::decay_t<U>>, int>>
  ...
  constexpr inline void back() { return _data[_index]; }

  constexpr inline void pop_back() {
    EOS_VM_ASSERT(_index >= 0, wasm_vector_oob_exception,
                  "vector pop out of bounds");
    _index--;
  }

  constexpr inline T &at(size_t i) {
    EOS_VM_ASSERT(i < _size, wasm_vector_oob_exception,
  ...
  constexpr inline T &at_no_check(size_t i) { return _data[i]; }

  constexpr inline T &at_no_check(size_t i) const { return _data[i]; }
