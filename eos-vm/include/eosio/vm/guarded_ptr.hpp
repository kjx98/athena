...
template <typename T> struct guarded_ptr {
  ...
  T *orig_ptr;
  T *bnds;
  guarded_ptr(T *rp, size_t bnds_)
      : raw_ptr(rp), orig_ptr(rp), bnds(rp + bnds_) {}

  inline guarded_ptr &operator+=(size_t i) {
    EOS_VM_ASSERT(i <= static_cast<std::size_t>(bnds - raw_ptr),
                  guarded_ptr_exception, "overbounding pointer");
    raw_ptr += i;
    return *this;
  }

  inline guarded_ptr &operator++() {
    EOS_VM_ASSERT(raw_ptr < bnds, guarded_ptr_exception,
  ...
  inline T *raw() { return raw_ptr; }

  inline size_t offset() { return raw_ptr - orig_ptr; }

  // reduces the bounds for the lifetime of the returned object
  auto scoped_shrink_bounds(std::size_t n) {
    EOS_VM_ASSERT(n <= static_cast<std::size_t>(bnds - raw_ptr),
                  guarded_ptr_exception, "guarded ptr out of bounds");
    T *old_bnds = bnds;
    bnds = raw_ptr + n;
  ...
  auto scoped_consume_items(std::size_t n) {
    ...
    return scope_guard{[this, old_bnds, exceptions, x]() {
      EOS_VM_ASSERT(exceptions != std::uncaught_exceptions() || raw_ptr == bnds,
                    guarded_ptr_exception, "guarded_ptr not advanced");
      bnds = old_bnds;
    }};
  }

  inline size_t bounds() { return bnds - orig_ptr; }

  inline T at(size_t index) const {
    EOS_VM_ASSERT(index < static_cast<std::size_t>(bnds - raw_ptr),
                  guarded_ptr_exception, "accessing out of bounds");
