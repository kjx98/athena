...
public:
  ...
  inline bool validate(const table_type &tt) {
    ...
    return tt.limits.initial <= max_limit && max_not_exceeded &&
           validate(tt.limits);
  }
  ...
  inline bool validate(const memory_type &mt) {
    ...
    return mt.limits.initial <= max_limit && max_not_exceeded &&
           validate(mt.limits);
  }

  // validate a global type 3.2.5
  inline bool validate(const global_type &gt) { return true; }
};
} // namespace vm
} // namespace eosio
