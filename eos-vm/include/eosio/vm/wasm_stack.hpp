...
public:
  template <typename Alloc = Allocator,
            typename = std::enable_if_t<std::is_same_v<Alloc, nullptr_t>, int>>
  stack() : _store(ElemSz) {}

  template <typename Alloc = Allocator,
            typename = std::enable_if_t<!std::is_same_v<Alloc, nullptr_t>, int>>
  stack(Alloc &&alloc) : _store(alloc, ElemSz) {}
  ...
  ElemT &get(uint32_t index) const {
    EOS_VM_ASSERT(index <= _index, wasm_interpreter_exception,
                  "invalid stack index");
...
private:
  using base_data_store_t =
      std::conditional_t<std::is_same_v<Allocator, nullptr_t>,
                         unmanaged_vector<ElemT>,
...
using call_stack =
    stack<activation_frame, constants::max_call_depth + 1, bounded_allocator>;
