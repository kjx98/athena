...
template <typename Writer> class binary_parser {
public:
  ...
  int validate_utf8_code_point(wasm_code_ptr &code) {
    ...
    if (ch < 0x80) {
      ...
      EOS_VM_ASSERT((ch & 0xC0) == 0xC0, wasm_parse_exception,
                    "invalid utf8 encoding");
      unsigned char b2 = *code++;
      EOS_VM_ASSERT((b2 & 0xC0) == 0x80, wasm_parse_exception,
                    "invalid utf8 encoding");
      uint32_t code_point = (static_cast<uint32_t>(ch - 0xC0u) << 6u) +
                            (static_cast<uint32_t>(b2 - 0x80u));
      EOS_VM_ASSERT(0x80 <= code_point && code_point < 0x800,
      ...
      EOS_VM_ASSERT((b3 & 0xC0) == 0x80, wasm_parse_exception,
                    "invalid utf8 encoding");
      uint32_t code_point = (static_cast<uint32_t>(ch - 0xE0u) << 12u) +
      ...
      EOS_VM_ASSERT((b4 & 0xC0) == 0x80, wasm_parse_exception,
                    "invalid utf8 encoding");
      uint32_t code_point = (static_cast<uint32_t>(ch - 0xF0u) << 18u) +
  ...
  void parse_module(wasm_code_ptr &code_ptr, size_t sz, module &mod) {
    _mod = &mod;
    EOS_VM_ASSERT(parse_magic(code_ptr) == constants::magic,
                  wasm_parse_exception, "magic number did not match");
    EOS_VM_ASSERT(parse_version(code_ptr) == constants::version,
                  wasm_parse_exception, "version number did not match");
    ...
    for (;;) {
      ...
      EOS_VM_ASSERT(id == 0 || id > highest_section_id, wasm_parse_exception,
    ...
    EOS_VM_ASSERT(
        _mod->code.size() == _mod->functions.size(), wasm_parse_exception,
  ...
  void parse_import_entry(wasm_code_ptr &code, import_entry &entry) {
    ...
    case external_kind::Function:
      ...
      EOS_VM_ASSERT(type < _mod->types.size(), wasm_parse_exception,
                    "Invalid function type");
      break;
    default:
      EOS_VM_ASSERT(false, wasm_unsupported_import_exception,
  ...
  void parse_table_type(wasm_code_ptr &code, table_type &tt) {
    ...
    EOS_VM_ASSERT(tt.element_type == types::anyfunc, wasm_parse_exception,
                  "table must have type anyfunc");
    tt.limits.flags = (parse_varuint32(code) & 0x1);
    tt.limits.initial = parse_varuint32(code);
    if (tt.limits.flags) {
      tt.limits.maximum = parse_varuint32(code);
      EOS_VM_ASSERT(tt.limits.initial <= tt.limits.maximum,
                    wasm_parse_exception, "table max size less than min size");
    }
  ...
  void parse_global_variable(wasm_code_ptr &code, global_variable &gv) {
    ...
    EOS_VM_ASSERT(ct == types::i32 || ct == types::i64 || ct == types::f32 ||
                      ct == types::f64,
  ...
  void parse_memory_type(wasm_code_ptr &code, memory_type &mt) {
    ...
    if (mt.limits.flags) {
      ...
      EOS_VM_ASSERT(mt.limits.maximum >= mt.limits.initial,
                    wasm_parse_exception, "maximum must be at least minimum");
      EOS_VM_ASSERT(mt.limits.maximum <= 65536u, wasm_parse_exception,
                    "maximum memory out of range");
    }
  }
  ...
  void parse_export_entry(wasm_code_ptr &code, export_entry &entry) {
    ...
    switch (entry.kind) {
    case external_kind::Function:
      EOS_VM_ASSERT(entry.index < _mod->get_functions_total(),
                    wasm_parse_exception, "function export out of range");
      break;
    case external_kind::Table:
      EOS_VM_ASSERT(entry.index < _mod->tables.size(), wasm_parse_exception,
                    "table export out of range");
      break;
    case external_kind::Memory:
      EOS_VM_ASSERT(entry.index < _mod->memories.size(), wasm_parse_exception,
                    "memory export out of range");
      break;
    case external_kind::Global:
      EOS_VM_ASSERT(entry.index < _mod->globals.size(), wasm_parse_exception,
                    "global export out of range");
      break;
    default:
      EOS_VM_ASSERT(false, wasm_parse_exception, "Unknown export kind");
      break;
    }
  }
  ...
  void parse_func_type(wasm_code_ptr &code, func_type &ft) {
    ...
    for (size_t i = 0; i < param_types.size(); i++) {
      uint8_t pt = *code++;
      param_types.at(i) = pt;
      EOS_VM_ASSERT(pt == types::i32 || pt == types::i64 || pt == types::f32 ||
                        pt == types::f64,
                    wasm_parse_exception, "invalid function param type");
    }
    ft.param_types = std::move(param_types);
    ft.return_count = *code++;
    EOS_VM_ASSERT(ft.return_count < 2, wasm_parse_exception,
                  "invalid function return count");
    if (ft.return_count > 0) {
      uint8_t rt = *code++;
      ft.return_type = rt;
      EOS_VM_ASSERT(rt == types::i32 || rt == types::i64 || rt == types::f32 ||
                        rt == types::f64,
  ...
  void parse_elem_segment(wasm_code_ptr &code, elem_segment &es) {
    ...
    EOS_VM_ASSERT(es.index == 0, wasm_parse_exception,
  ...
  void parse_init_expr(wasm_code_ptr &code, init_expr &ie, uint8_t type) {
    ...
    case opcodes::i32_const:
      ...
      EOS_VM_ASSERT(type == types::i32, wasm_parse_exception,
                    "expected i32 initializer");
      break;
    case opcodes::i64_const:
      ie.value.i64 = parse_varint64(code);
      EOS_VM_ASSERT(type == types::i64, wasm_parse_exception,
                    "expected i64 initializer");
      break;
    case opcodes::f32_const:
      ie.value.f32 = parse_raw<uint32_t>(code);
      EOS_VM_ASSERT(type == types::f32, wasm_parse_exception,
                    "expected f32 initializer");
      break;
    case opcodes::f64_const:
      ie.value.f64 = parse_raw<uint64_t>(code);
      EOS_VM_ASSERT(type == types::f64, wasm_parse_exception,
                    "expected f64 initializer");
      break;
    default:
      EOS_VM_ASSERT(false, wasm_parse_exception,
                    "initializer expression can only acception i32.const, "
                    "i64.const, f32.const and f64.const");
    }
    EOS_VM_ASSERT((*code++) == opcodes::end, wasm_parse_exception,
                  "no end op found");
  }
  ...
  void parse_function_body(wasm_code_ptr &code, function_body &fb,
    ...
    for (size_t i = 0; i < local_cnt; i++) {
      ...
      if (count == 0)
        type = types::i32;
      EOS_VM_ASSERT(type == types::i32 || type == types::i64 ||
                        type == types::f32 || type == types::f64,
    ...
    code += fb.size - 1;
    EOS_VM_ASSERT(*code++ == 0x0B, wasm_parse_exception,
  ...
  struct operand_stack_type_tracker {
    ...
    void push(uint8_t type) {
      ...
      assert(type == types::i32 || type == types::i64 || type == types::f32 ||
             type == types::f64 || type == any_type);
      EOS_VM_ASSERT(operand_depth < std::numeric_limits<uint32_t>::max(),
                    wasm_parse_exception, "integer overflow in operand depth");
      ++operand_depth;
    ...
    void pop(uint8_t expected) {
      ...
      if (state.back() != unreachable_tag) {
        EOS_VM_ASSERT(state.back() == expected || state.back() == any_type,
                      wasm_parse_exception, "wrong type");
        --operand_depth;
        state.pop_back();
      }
    }
    uint8_t pop() {
      EOS_VM_ASSERT(!state.empty() && state.back() != scope_tag,
                    wasm_parse_exception, "unexpected pop");
      if (state.back() == unreachable_tag)
        return any_type;
      else {
    ...
    void push_scope() { state.push_back(scope_tag); }
    void pop_scope(uint8_t expected_result = types::pseudo) {
      ...
      EOS_VM_ASSERT(state.back() == scope_tag, wasm_parse_exception,
                    "unexpected end");
      state.pop_back();
    ...
    void finish() {
      ...
      EOS_VM_ASSERT(state.empty(), wasm_parse_exception,
                    "stack not empty at scope end");
    }
    uint32_t depth() const { return operand_depth; }
  };

  struct local_types_t {
    local_types_t(const func_type &ft,
                  const guarded_vector<local_entry> &locals_arg)
      ...
      for (uint32_t i = 0; i < locals_arg.size(); ++i) {
        ...
        EOS_VM_ASSERT(count <= 0xFFFFFFFFu - locals_arg[i].count,
                      wasm_parse_exception, "too many locals");
        count += locals_arg[i].count;
        _boundaries.push_back(count);
      }
    }
    uint8_t operator[](uint32_t local_idx) const {
      EOS_VM_ASSERT(local_idx < _boundaries.back(), wasm_parse_exception,
                    "undefined local");
      auto pos =
          std::upper_bound(_boundaries.begin(), _boundaries.end(), local_idx);
  ...
  void parse_function_body_code(wasm_code_ptr &code, size_t bounds,
                                Writer &code_writer, const func_type &fnt,
                                const local_types_t &local_types) {
    // Initialize the control stack with the current function as the sole
    // element
    operand_stack_type_tracker op_stack;
    std::vector<pc_element_t> pc_stack{
        {op_stack.depth(),
         fnt.return_count ? fnt.return_type
                          : static_cast<uint32_t>(types::pseudo),
         fnt.return_count ? fnt.return_type
                          : static_cast<uint32_t>(types::pseudo),
         false, std::vector<branch_t>{}}};
    ...
    auto handle_branch_target = [&](uint32_t label, branch_t address) {
      EOS_VM_ASSERT(label < pc_stack.size(), wasm_parse_exception,
                    "invalid label");
      pc_element_t &branch_target = pc_stack[pc_stack.size() - label - 1];
      std::visit(overloaded{[&](label_t target) {
                              code_writer.fix_branch(address, target);
                            },
                            [&](std::vector<branch_t> &relocations) {
    ...
    auto exit_scope = [&]() {
      ...
      EOS_VM_ASSERT(pc_stack.size(), wasm_invalid_element,
                    "unexpected end instruction");
      // an if with an empty else cannot have a return value
      EOS_VM_ASSERT(!pc_stack.back().is_if ||
                        pc_stack.back().expected_result == types::pseudo,
                    wasm_parse_exception, "wrong type");
      auto end_pos = code_writer.emit_end();
      if (auto *relocations = std::get_if<std::vector<branch_t>>(
              &pc_stack.back().relocations)) {
    ...
    while (code.offset() < bounds) {
      EOS_VM_ASSERT(pc_stack.size() <= constants::max_nested_structures,
      ...
      case opcodes::end: {
        exit_scope();
        EOS_VM_ASSERT(!pc_stack.empty() || code.offset() == bounds,
                      wasm_parse_exception, "function too short");
        break;
      }
      ...
      case opcodes::block: {
        ...
        EOS_VM_ASSERT(expected_result == types::i32 ||
                          expected_result == types::i64 ||
                          expected_result == types::f32 ||
                          expected_result == types::f64 ||
                          expected_result == types::pseudo,
                      wasm_parse_exception, "Invalid type code in block");
        pc_stack.push_back({op_stack.depth(), expected_result, expected_result,
                            false, std::vector<branch_t>{}});
      ...
      case opcodes::loop: {
        ...
        EOS_VM_ASSERT(expected_result == types::i32 ||
                          ...
                          expected_result == types::f64 ||
                          expected_result == types::pseudo,
                      wasm_parse_exception, "Invalid type code in loop");
        auto pos = code_writer.emit_loop();
        pc_stack.push_back(
            {op_stack.depth(), expected_result, types::pseudo, false, pos});
        op_stack.push_scope();
      } break;
      case opcodes::if_: {
        uint32_t expected_result = *code++;
        ...
        EOS_VM_ASSERT(expected_result == types::i32 ||
                          ...
                          expected_result == types::f64 ||
                          expected_result == types::pseudo,
                      wasm_parse_exception, "Invalid type code in if");
        auto branch = code_writer.emit_if();
        op_stack.pop(types::i32);
        pc_stack.push_back({op_stack.depth(), expected_result, expected_result,
                            true, std::vector{branch}});
        op_stack.push_scope();
      } break;
      case opcodes::else_: {
        ...
        auto &relocations =
            std::get<std::vector<branch_t>>(old_index.relocations);
      ...
      case opcodes::br_table: {
        ...
        for (size_t i = 0; i < table_size; i++) {
          ...
          handle_branch_target(label, branch);
          uint8_t one_result =
              pc_stack[pc_stack.size() - label - 1].label_result;
          if (i == 0) {
            result_type = one_result;
          } else {
            EOS_VM_ASSERT(result_type == one_result, wasm_parse_exception,
      ...
      case opcodes::call: {
        ...
        EOS_VM_ASSERT(ft.return_count <= 1, wasm_parse_exception,
                      "unsupported");
        if (ft.return_count)
      ...
      case opcodes::call_indirect: {
        ...
        EOS_VM_ASSERT(_mod->tables.size() > 0, wasm_parse_exception,
        ...
        EOS_VM_ASSERT(*code == 0, wasm_parse_exception,
      ...
      case opcodes::select: {
        ...
        EOS_VM_ASSERT(t0 == t1 || t0 == any_type || t1 == any_type,
                      wasm_parse_exception, "incorrect types for select");
      ...
      case opcodes::set_global: {
        uint32_t global_idx = parse_varuint32(code);
        EOS_VM_ASSERT(_mod->globals.at(global_idx).type.mutability,
                      wasm_parse_exception, "cannot set const global");
...
#define LOAD_OP(op_name, max_align, type)                                      \
  case opcodes::op_name: {                                                     \
    EOS_VM_ASSERT(_mod->memories.size() > 0, wasm_parse_exception,             \
                  "load requires memory");                                     \
    uint32_t alignment = parse_varuint32(code);                                \
    uint32_t offset = parse_varuint32(code);                                   \
    EOS_VM_ASSERT(alignment <= uint32_t(max_align), wasm_parse_exception,      \
                  "alignment cannot be greater than size.");                   \
    op_stack.pop(types::i32);                                                  \
...
#undef BINOP
      case opcodes::error:
        code_writer.emit_error();
        break;
      }
    }
    EOS_VM_ASSERT(pc_stack.empty(), wasm_parse_exception,
                  "function body too long");
    _mod->maximum_stack =
        std::max(_mod->maximum_stack,
                 static_cast<uint64_t>(op_stack.maximum_operand_depth) +
  ...
  inline void parse_section_impl(wasm_code_ptr &code, vec<Elem> &elems,
  ...
  inline void parse_section(
      wasm_code_ptr &wcode,
      vec<typename std::enable_if_t<id == section_id::type_section, func_type>>
          &elems) {
    parse_section_impl(wcode, elems,
                       [&](wasm_code_ptr &code, func_type &ft,
                           std::size_t /*idx*/) { parse_func_type(code, ft); });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::import_section,
                                              import_entry>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, import_entry &ie, std::size_t /*idx*/) {
          parse_import_entry(code, ie);
        });
  }
  template <uint8_t id>
  inline void parse_section(
      wasm_code_ptr &wcode,
      vec<typename std::enable_if_t<id == section_id::function_section,
                                    uint32_t>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, uint32_t &elem, std::size_t /*idx*/) {
          elem = parse_varuint32(code);
        });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::table_section,
                                              table_type>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, table_type &tt, std::size_t /*idx*/) {
          parse_table_type(code, tt);
        });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::memory_section,
                                              memory_type>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, memory_type &mt, std::size_t idx) {
          EOS_VM_ASSERT(idx == 0, wasm_parse_exception,
  ...
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::global_section,
                                              global_variable>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, global_variable &gv, std::size_t /*idx*/) {
          parse_global_variable(code, gv);
        });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::export_section,
                                              export_entry>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, export_entry &ee, std::size_t /*idx*/) {
          parse_export_entry(code, ee);
        });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &code,
                typename std::enable_if_t<id == section_id::start_section,
                                          uint32_t> &start) {
  ...
  inline void
  parse_section(wasm_code_ptr &code,
                vec<typename std::enable_if_t<id == section_id::element_section,
                                              elem_segment>> &elems) {
    parse_section_impl(
        code, elems,
        [&](wasm_code_ptr &wcode, elem_segment &es, std::size_t /*idx*/) {
          parse_elem_segment(wcode, es);
        });
  }
  template <uint8_t id>
  inline void
  parse_section(wasm_code_ptr &code,
                vec<typename std::enable_if_t<id == section_id::code_section,
                                              function_body>> &elems) {
    parse_section_impl(
        code, elems,
        [&](wasm_code_ptr &wcode, function_body &fb, std::size_t idx) {
          parse_function_body(wcode, fb, idx);
        });
    EOS_VM_ASSERT(
        elems.size() == _mod->functions.size(), wasm_parse_exception,
        "code section must have the same size as the function section");
  ...
  inline void
  parse_section(wasm_code_ptr &wcode,
                vec<typename std::enable_if_t<id == section_id::data_section,
                                              data_segment>> &elems) {
    parse_section_impl(
        wcode, elems,
        [&](wasm_code_ptr &code, data_segment &ds, std::size_t /*idx*/) {
          parse_data_segment(code, ds);
        });
  }
